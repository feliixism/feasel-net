...
class FeatureSelection(Callback):
  ...
  def map_evaluation_data(self):
    ...
    if self._params.n_samples != 0:
      ...
      for i, j in enumerate(idx):
        ...
        # evaluation_data = I[j] * self.data_tf
        evaluation_data[:, j] = 0
  ...
  def merge_losses(self, H):
    ...
    if self.log.f_loss:
      ...
      # if self._params.rationalize: # not available in Version 1.0.0
      #   H_previous = self._rationalize_loss(H_previous)
  ...
  def _get_information_richness(self):
    ...
    if self._params.eval_normalization == 'min-max':
      H = prep.min_max(H, axis=0)

    elif self._params.eval_normalization == 'standardize':
      H = prep.standardize(H, axis=0)
