...
class NeuralNetworkVisualizer(Base):
  ...
  def confusion_matrix(self, X, y, normalize=True):
    ...
    y_pred = (y_pred==np.amax(y_pred, axis=1, keepdims=True)).astype(float)

    # normalized confusion matrix:
    cm = np.zeros([y_pred.shape[-1], y_pred.shape[-1]])
    for i in range(y_pred.shape[-1]):
      for j in range(y_true.shape[-1]):
        if normalize:
          cm[j,i] = np.sum(y_pred[:,i] * y_true[:,j]) / np.sum(y_true[:,j]) * 100
        else:
          cm[j,i] = np.sum(y_pred[:,i] * y_true[:,j])
