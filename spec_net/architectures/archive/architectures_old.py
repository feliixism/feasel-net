...
import numpy as np
import keras
from keras.layers import Input, Dense, BatchNormalization, Lambda, Concatenate, Reshape
...
class Base:
    def __init__(self, x_train, y_train):
        ...
        self.optimizer = keras.optimizers.Adam(lr = 0.000001)
...
class ANN(Base):
    ...
    def get_block(self, x, n_layers = 3):
        for i in range(1, n_layers + 1):
            x = Dense(int(self.x_train.shape[1] / 2 ** (2*i)), activation = "relu", name = f"ClassifierDense{i}")(x)
