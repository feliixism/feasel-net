...
class Classification:
    def __init__(self, X, y, features = None, **kwargs):
        ...
        #params container
        self._data_params = {"sample_axis": None,
                             "input_layer_type": None,
                             "normalization": None}
        
        for key in kwargs:
            if key in self._data_params:
                self._data_params[f"{key}"] = kwargs[f"{key}"]
        
        self._params = {"data": self._data_params}
        
        self.prepare_data()
        
    def __repr__(self):
        return f"Data(Size of Dataset: {self.X.shape}, Number of Samples: {self.n_samples}, Number of Classes: {self.n_classes})"
    ...
    def prepare_data(self):
        ...
        if self.y is not None:
            ...
            #get feature shape
            if self._data_params["sample_axis"] is None:
                sample_axis = int(np.argwhere(np.array(self.X.shape) == self.n_samples))
            else:
                sample_axis = self._data_params["sample_axis"]
            ...
            if self.n_samples != len(self.y):
                raise ValueError(f"Number of samples given in 'sample_axis' ({self.n_samples}) does not match with samples in given in 'y' ({len(self.y)}). Please change your sample axis.")
        
        else:
            if self._data_params["sample_axis"]:
        ...
        if self._data_params["normalization"] is not None:
            if self._data_params["normalization"] == "standardize":
                self.standardize()
            elif self._data_params["normalization"] == "min_max":
                self.min_max_scale()
            else:
                raise NameError(f"'{self._data_params['normalization']}' is not implemented as a normalization technique.")
        
        if self._data_params["input_layer_type"] is not None:
            self.convert_to_input_shape(self._data_params["input_layer_type"])
    
    def convert_to_input_shape(self, input_layer_type):
        """
        ...
        layer_dims = {"Dense": 2,
                      ...
                      "Conv3D": 5}
        try:
            input_layer_dim = layer_dims[f"{input_layer_type}"]
            
            while True:
                X_dim = self.X.ndim    
                
                #expansion of dimensionality by adding another dimension after the previous last dimension
                if input_layer_dim > X_dim:
                    self.X = np.expand_dims(self.X, axis = -1)
                
                #reduction of dimensionality by multiplying the last two dimensionalities as the new reduced dimension 
                elif input_layer_dim < X_dim:
                    shape = (self.n_samples, ) + self.X.shape[1:-2] + (self.X.shape[-2] * self.X.shape[-1], )
                    self.X = self.X.reshape(shape)
                else:
                    break
            self.feature_shape = self.X.shape[1:]
        except:
            raise NameError(f"{input_layer_type} is either not a possible keras input layer or not implemented yet. Please try one of the following layers: {layer_dims.keys()}.")
    
    def sparse_labels(self):
        self.y_train = df.sparse_labels(self.y_train)
        return self.y_train
    
    def one_hot_labels(self):
        self.y_train = df.one_hot_labels(self.y_train)
        return self.y_train
    ...
    def min_max_scale(self, a_max = 1, a_min = 0):
        self.X_train = df.min_max_scale(self.X_train, axis = -1, a_max = a_max, a_min = a_min)
