...
from .constraints import MinMaxClip
...
# model operations    
    ...
    def get_model(self, *class_objects):
        ...
        for i in range(len(class_objects)):
            ...
            outputs.append(class_objects[i].output_layer)
    ...
    def fit_model(self, *class_objects):
        ...
        history = self.model.fit(x = x, y = y, epochs = self.epochs, batch_size = self.batch_size, callbacks = [callback], validation_split = self.validation_split)
        return history
...
class ANN(Base):
    def __init__(self, x_train, y_train, loss_weight = 1.0):
        ...
        self.x_train, self.y_train = self._convert_data(x_train, y_train)
        self.n_layers = 3
        self.n_nodes = self.n_labels * 2**(self.n_layers + 1)
        self.get_architecture()
        
    def get_architecture(self):
        
        self.input_layer = x = Input(shape = (self.x_train.shape[1], ), name = "ClassifierIn")
    ...
    def get_block(self, x):
        for i in range(1, self.n_layers + 1):
            x = Dense(int(self.n_nodes / 2**i), activation = "relu", name = f"ClassifierDense{i}")(x)
            x = BatchNormalization(momentum = 0.0, name = f"ClassifierBatchNorm{i}")(x)
        return x
...
class CNN_1D(Base):
    ...
    def get_architecture(self):
        self.input_layer = x = Input(shape = (self.x_train.shape[1], self.x_train.shape[2]), name = "ClassifierIn")
    ...
    def get_block(self, x, n_layers = 3):
        for i in range(1, n_layers + 1):
            x = Conv1D(int(12 / i), 3 * i, activation = "relu", padding = "same", name = f"ClassifierConv1d{i}")(x)
            x = BatchNormalization(momentum = 0.0, name = f"ClassifierBatchNorm{i}")(x)
            x = MaxPooling1D(name = f"ClassifierMaxPool1d{i}")(x)
            # x = Dropout(self.dropout_rate, name = f"classifier_dropout_{i}")(x)
            
        x = Flatten(name = "ClassifierFlatten")(x)
        x = Dropout(self.dropout_rate, name = f"ClassifierDropout")(x)
        x = Dense(self.n_labels * 2, activation='relu', name = "ClassifierDense")(x)
        return x
